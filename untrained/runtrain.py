...
def train_network(use_gpu=True, restore_if_possible=True, batch_size=30):
    with tf.device("/cpu:0"):
        ...
        correct_mnist = tf.placeholder(tf.float32, shape=(batch_size, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS))
        mismatch_mnist = tf.placeholder(tf.float32, shape=(batch_size, 9, IMAGE_SIZE * IMAGE_SIZE * NUM_CHANNELS))
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            summary_writer = tf.summary.FileWriter('tflog3', sess.graph)  # For logging for TensorBoard
