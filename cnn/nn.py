...
def conv_layer(height, channels, name='conv-layer', padding='VALID'):
    def make_layer(input_to_layer):
        # print("Conv1", input_to_layer.get_shape())
        with tf.variable_scope(name, values=[input_to_layer]):
            # with tf.name_scope('weights'):
                weights = weight_variable([5, input_to_layer.get_shape()[2], input_to_layer.get_shape()[3], channels], stddev=5e-2)
...
def max_pool_layer(name='max-pool-layer'):
    def make_layer(input_to_layer):
...
def avg_pool_layer(name='mean-pool-layer'):
    def make_layer(input_to_layer):
...
def norm_layer(name='norm-layer'):
    def make_layer(input_to_layer):
